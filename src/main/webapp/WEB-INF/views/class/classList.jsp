...
<section class="panel">
    ...
    <c:set var="toolbarAction" value="/classes" />
    <c:set var="toolbarScope" value="class" />
    <c:set var="toolbarButtonHref" value="/classes/new" />
    ...
    <div class="pagination">
        <c:if test="${classPage.hasPrevious}">
            <a href="/classes?page=${classPage.startPage - 1}&keyField=${selectedKeyField}&keyword=${keyword}&onoff=${selectedOnOff}">이전</a>
        </c:if>
        <c:forEach begin="${classPage.startPage}" end="${classPage.endPage}" var="pageNum">
            <a class="${pageNum == classPage.currentPage ? 'active' : ''}" href="/classes?page=${pageNum}&keyField=${selectedKeyField}&keyword=${keyword}&onoff=${selectedOnOff}">${pageNum}</a>
        </c:forEach>
        <c:if test="${classPage.hasNext}">
            <a href="/classes?page=${classPage.endPage + 1}&keyField=${selectedKeyField}&keyword=${keyword}&onoff=${selectedOnOff}">다음</a>
        </c:if>
    </div>
</section>
