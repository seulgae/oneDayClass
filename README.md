...
추가 권한 정책:
- 일반회원 `level 1`: 수강/결제/일반 게시판 사용
- 강사 `level 2`: 클래스 등록 가능
- 관리자 `level 3`: 게시판 관리, 관리자 대시보드 접근 가능
- 운영자 `level 4`: 게시판 관리, 관리자 대시보드 접근 가능
- 후기/QnA/요청 게시판 삭제는 작성자 또는 게시판 관리자(`level 3`, `level 4`)가 가능
- 게시판 삭제는 물리 삭제가 아니라 상태값 변경(소프트 삭제) 방식
...
- 관리자 1명, 일반 회원 1명, 강사 1명이 기본 시드로 들어갑니다.
- 클래스, 공지성 QnA, 일반 QnA, 리뷰, 등업 신청, 요청 게시판 샘플 데이터가 포함되어 있습니다.
- 후기는 `reviewBBS.cNum`으로 클래스와 연결되며, 목록/상세에서 연결된 클래스명을 함께 표시합니다.
...
- 회원 등급은 일반회원 `1`, 강사 `2`, 관리자 `3`, 운영자 `4`를 사용합니다.
